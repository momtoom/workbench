/**
 * Dynamic CSF story / component loader shared by:
 * - the Storybook surface (preview canvas)
 * - the Design Editor's component picker (preview thumbnail)
 *
 * The metadata-only sibling lives in `sourceStoryMetadata.ts`. This module
 * additionally resolves a real executable `render` function so callers can
 * actually mount the component, not just read its prop schema.
 */
...
export type CsfRuntimeStorySourceComponent = {
  id: string;
  name: string;
  sourceFile: string;
...
export type CsfRuntimeImportResult = {
  component: ComponentType<WorkbenchStoryArgs>;
  story: WorkbenchStory;
};

export type CsfRuntimeModuleState =
  | { status: 'idle'; component: null; diagnostic: string }
  | { status: 'loading'; component: null; diagnostic: string }
  | { status: 'ready'; component: ComponentType<WorkbenchStoryArgs>; diagnostic: string }
  | { status: 'error'; component: null; diagnostic: string };

export async function importStorybookCsfStory(
  sourceFile: string,
  component: CsfRuntimeStorySourceComponent,
): Promise<CsfRuntimeImportResult | null> {
  const errors: string[] = [];
  for (const candidate of getStoryRuntimeImportCandidates(sourceFile)) {
    try {
      ...
      const built = createWorkbenchStoryFromCsfModule(module, component);
      if (built) return built;
    } catch (error) {
      errors.push(`${candidate}: ${formatRuntimeImportError(error)}`);
      // Try the next extension/index fallback.
    }
  }
  const fallback = await importStorybookComponentSourceFallback(component);
  if (fallback) return fallback;
  if (errors.length > 0) {
    console.warn(`[workbench] CSF story runtime import failed for ${sourceFile}: ${errors.join(' | ')}`);
  }
  return null;
}

async function importStorybookComponentSourceFallback(
  component: CsfRuntimeStorySourceComponent,
): Promise<CsfRuntimeImportResult | null> {
  const exportNames = getCsfRuntimeComponentExportCandidates(component);
  for (const candidate of getStoryRuntimeImportCandidates(component.sourceFile)) {
    try {
      const moduleUrl = await resolveWorkbenchRuntimeModuleUrl(candidate);
      if (!moduleUrl) continue;
      const module = await import(/* @vite-ignore */ moduleUrl) as Record<string, unknown>;
      for (const exportName of exportNames) {
        const exported = exportName === 'default' ? module.default : module[exportName];
        if (typeof exported === 'function') {
          return createSourceFallbackWorkbenchStory(component, exported as ComponentType<WorkbenchStoryArgs>, exportName);
        }
      }
    } catch {
      // Try the next source extension/index fallback.
    }
  }
  return null;
}

function getCsfRuntimeComponentExportCandidates(component: CsfRuntimeStorySourceComponent): string[] {
  return [...new Set([
...
function createSourceFallbackWorkbenchStory(
  component: CsfRuntimeStorySourceComponent,
  componentExport: ComponentType<WorkbenchStoryArgs>,
  exportName: string,
): CsfRuntimeImportResult {
  const render = (args: WorkbenchStoryArgs) => createElement(componentExport, args);
  const story: WorkbenchStory = {
    componentId: component.id,
    controls: [],
    defaultArgs: {},
    description: `Source preview fallback for ${component.sourceFile}.`,
    name: component.name,
    previewExportName: exportName,
    previewSourceFile: component.sourceFile,
    render,
    variants: [],
  };
  return { component: (args: WorkbenchStoryArgs) => render(args) as ReactNode, story };
}

export async function importStorybookRuntimeComponent(
  sourceFile: string,
  exportName: string,
): Promise<CsfRuntimeModuleState> {
  const errors: string[] = [];
  for (const candidate of getStoryRuntimeImportCandidates(sourceFile)) {
    try {
      const moduleUrl = await resolveWorkbenchRuntimeModuleUrl(candidate);
      ...
      if (typeof exported === 'function') {
        return {
          status: 'ready',
          component: exported as ComponentType<WorkbenchStoryArgs>,
          diagnostic: `Loaded ${sourceFile}.`,
        };
      }
      return { status: 'error', component: null, diagnostic: `${sourceFile} does not export ${exportName}.` };
    } catch (error) {
      errors.push(`${candidate}: ${formatRuntimeImportError(error)}`);
      // Try the next extension/index fallback.
    }
  ...
  return {
    status: 'error',
    component: null,
    diagnostic: `${sourceFile} could not be loaded for runtime preview.${errors.length > 0 ? ` ${errors.join(' | ')}` : ''}`,
  };
}
...
function createWorkbenchStoryFromCsfModule(
  module: Record<string, unknown>,
  component: CsfRuntimeStorySourceComponent,
): CsfRuntimeImportResult | null {
  ...
  const render = resolveCsfRender(meta, primaryExport);
  ...
  const story: WorkbenchStory = {
    ...
    name: getTrimmedString(isRecord(primaryExport) ? primaryExport.name : undefined) ?? component.name,
    render,
...
function createDesignStoryContract(
  ...
  const designArgs = normalizeWorkbenchStoryDesignDefaultArgs(sourceInsert.props ?? {}, component);
  ...
  const sourceInsertControls = Object.keys(designArgs)
    ...
    .map((key) => (
      controlByKey.get(key) ?? createFallbackControl(key, designArgs[key])
    ));
...
function shouldExposeAllCsfControlsForDesign(component: CsfRuntimeStorySourceComponent): boolean {
...
function normalizeWorkbenchStoryDesignDefaultArgs(
  ...
  const componentName = getShadcnBaseComponentName(component);
  return sortWorkbenchStoryArgs(Object.fromEntries(Object.entries(defaultArgs).map(([key, value]) => (
    typeof value === 'string'
      ? [key, getShadcnBaseNeutralText(componentName, key, value)]
      : [key, value]
  ))));
}

function getShadcnBaseComponentName(component: CsfRuntimeStorySourceComponent): string {
  return getTrimmedString(component.extensions?.importName) ??
    getTrimmedString(component.extensions?.sourceExportName) ??
    component.name;
}
...
function getShadcnBaseNeutralText(componentName: string, key: string, value: string): string {
  ...
  if (normalizedKey === 'title' || normalizedKey.endsWith('title')) return 'Title';
  if (normalizedKey === 'description' || normalizedKey.endsWith('description')) return 'Description';
...
function createFallbackControl(key: string, value: WorkbenchStoryArgValue): WorkbenchStoryControl {
  const knownControl = createValueTypeFallbackControl(key, value, formatCsfArgLabel(key));
  if (knownControl) return knownControl;
  if (isIconLikeControlKey(key, value)) return { key, label: formatCsfArgLabel(key), type: 'icon' };
  return { key, label: formatCsfArgLabel(key), type: 'text' };
}
...
function getCsfComponentNameCandidates(component: CsfRuntimeStorySourceComponent): Set<string> {
  return new Set([
    component.name,
    getTrimmedString(component.extensions?.importName),
    getTrimmedString(component.extensions?.sourceExportName),
...
function resolveCsfRender(
  meta: Record<string, unknown>,
  storyExport: unknown,
): ((args: WorkbenchStoryArgs) => ReactNode) | null {
  if (typeof storyExport === 'function') {
    return (args) => createElement(storyExport as ComponentType<WorkbenchStoryArgs>, args);
  }
  if (isRecord(storyExport) && typeof storyExport.render === 'function') {
    return storyExport.render as (args: WorkbenchStoryArgs) => ReactNode;
  }
  if (typeof meta.component === 'function') {
    return (args) => createElement(meta.component as ComponentType<WorkbenchStoryArgs>, args);
  }
  return null;
}
...
function normalizeCsfControls(argTypes: unknown, defaultArgs: WorkbenchStoryArgs): WorkbenchStoryControl[] {
  if (!isRecord(argTypes)) {
    return sortWorkbenchStoryControls(Object.entries(sortWorkbenchStoryArgs(defaultArgs))
      .filter(([key]) => !isSourceStylePropKey(key))
      .map(([key, value]) => (
  ...
  return sortWorkbenchStoryControls(Object.entries(argTypes).flatMap(([key, value]): WorkbenchStoryControl[] => {
...
function normalizeCsfControlSuggestions(value: unknown): string[] | undefined {
  if (!Array.isArray(value)) return undefined;
  const suggestions = dedupeStrings(value.filter((item): item is string => typeof item === 'string'));
...
function isIconLikeControlKey(key: string, value: WorkbenchStoryArgValue | undefined): boolean {
