...
function Menubar({ className, ...props }: MenubarPrimitive.Props) {
  return (
    <MenubarPrimitive
      data-slot="menubar"
      className={cn(
        "flex h-8 w-fit items-center gap-0.5 rounded-lg border p-0.75",
        className
      )}
      {...props}
    />
...
type MenubarMenuProps = Omit<React.ComponentProps<typeof DropdownMenu>, "trigger"> & {
...
function MenubarMenu({
  ...
  const simpleModeChildren = getMenubarMenuSimpleModeChildren(children as React.ReactNode)
...
type MenubarRadioSectionProps = Omit<
  React.ComponentProps<typeof MenubarRadioGroup>,
  "children"
> & {
...
function MenubarRadioSection({
  ...
  defaultValue = "item-1",
  ...props
}: MenubarRadioSectionProps) {
  return (
    <MenubarRadioGroup defaultValue={defaultValue} {...props}>
      {children}
    </MenubarRadioGroup>
  )
}
...
type MenubarSubmenuProps = Omit<React.ComponentProps<typeof MenubarSub>, "children"> & {
  children?: React.ReactNode
  contentClassName?: string
  inset?: boolean
  trigger?: React.ReactNode
...
function MenubarSubmenu({
  ...
  return (
    <MenubarSub {...componentProps}>
      ...
      <MenubarSubContent className={contentClassName}>
        {children}
      </MenubarSubContent>
    </MenubarSub>
  )
...
const MENUBAR_MENU_SIMPLE_MODE_CHILD_TYPES = new Set<React.ElementType>([
  MenubarGroup,
  MenubarLabel,
  MenubarItem,
  MenubarCheckboxItem,
...
function getMenubarMenuSimpleModeChildren(children: React.ReactNode): React.ReactNode[] | null {
  const childArray = React.Children.toArray(children).filter(isMeaningfulMenubarMenuChild)
  if (childArray.length === 0) return null
  if (!childArray.every(isMenubarMenuSimpleModeElement)) return null
  return childArray
}

function isMeaningfulMenubarMenuChild(child: React.ReactNode): boolean {
  return !(typeof child === "string" && child.trim() === "")
}

function isMenubarMenuSimpleModeElement(child: React.ReactNode): child is React.ReactElement {
  return isWorkbenchElementOfTypes(child, MENUBAR_MENU_SIMPLE_MODE_CHILD_TYPES)
}
